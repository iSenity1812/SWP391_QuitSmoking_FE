...
export function useMonthlyUsersData() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.log('📊 [useMonthlyUsersData] Fetching users data...')
      ...
      console.log('✅ [useMonthlyUsersData] Data processed successfully:', processedData.length, 'days')
    } catch (err) {
      console.error('❌ [useMonthlyUsersData] Error fetching data:', err)
  ...
  const refetch = useCallback(() => {
    console.log('🔄 [useMonthlyUsersData] Manual refetch triggered')
    fetchData()
  }, [fetchData])
