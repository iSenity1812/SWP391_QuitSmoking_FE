...
import { DateRangeRegistrationModal } from "@/components/coach/DateRangeRegistrationModal"
...
interface WeekNavigationProps {
  ...
  setIsDateRangeRegistrationOpen: (open: boolean) => void
  onRefresh: () => void
  isRefreshing?: boolean
}

function WeekNavigation({ currentWeekStart, onWeekChange, setIsRegistrationOpen, setIsDateRangeRegistrationOpen, onRefresh, isRefreshing = false }: WeekNavigationProps) {
    ...
    <div className="flex items-center justify-between mb-6">
      <div className="flex items-center space-x-4">
        ...
        <Button
          variant="outline"
          size="sm"
          className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border-blue-200 dark:border-blue-700 hover:from-blue-100 hover:to-blue-200 dark:hover:from-blue-900/30 dark:hover:to-blue-800/30"
          onClick={() => setIsDateRangeRegistrationOpen(true)}
        >
          <Calendar className="w-4 h-4 mr-2" />
          Đăng ký theo khoảng thời gian
        </Button>
      </div>
    </div>
  )
...
export function WeeklyScheduleTable() {// Get current coach ID from auth context
  ...
  const [isRefreshing, setIsRefreshing] = useState(false)  // Use custom hooks for API data
  ...
  const [isDateRangeRegistrationOpen, setIsDateRangeRegistrationOpen] = useState(false)
  ...
  // Helper function to check if appointment can be joined for meeting
  const canJoinMeeting = (appointment: { status: string, appointmentId: number }, date: string, timeSlot: TimeSlot): { canJoin: boolean; reason?: string } => {
    const now = new Date();
    ...
    // Cannot join cancelled appointments
    if (appointment.status === 'cancelled') {
      return { canJoin: false, reason: 'Không thể tham gia cuộc hẹn đã bị hủy' };
    }

    // Can join confirmed and scheduled appointments
    if (!['confirmed', 'scheduled'].includes(appointment.status)) {
      return { canJoin: false, reason: 'Chỉ có thể tham gia cuộc hẹn đã được xác nhận hoặc đã lên lịch' };
    }

    // Can join 15 minutes before appointment starts until 15 minutes after it ends
    const joinStartTime = new Date(appointmentStartTime.getTime() - 15 * 60 * 1000); // 15 minutes before
    const joinEndTime = new Date(appointmentEndTime.getTime() + 15 * 60 * 1000); // 15 minutes after

    if (now < joinStartTime) {
      const minutesUntilJoin = Math.ceil((joinStartTime.getTime() - now.getTime()) / (60 * 1000));
      return { canJoin: false, reason: `Có thể tham gia sau ${minutesUntilJoin} phút nữa` };
    }

    if (now > joinEndTime) {
      return { canJoin: false, reason: 'Cuộc hẹn đã kết thúc' };
    }
  ...
  const handleJoinMeeting = async (appointmentId: number) => {
    try {
      setIsJoiningMeeting(appointmentId);

      // Get Agora token for the coach (isPublisher = true)
      await agoraService.getAgoraToken(appointmentId, true);

      // Navigate to meeting page with the appointment ID
      // You can either use React Router navigation or open in new window
      const meetingUrl = `/meeting/${appointmentId}`;
      window.open(meetingUrl, '_blank');

      toast.success('Đang mở phòng meeting...', {
        position: "top-right",
        autoClose: 3000,
      });
      ...
      toast.error('Không thể tham gia cuộc hẹn. Vui lòng thử lại.', {
        position: "top-right",
        autoClose: 5000,
      });
    } finally {
  ...
  return (
    <div className="space-y-6">      {/* Week Navigation */}
      <WeekNavigation
        ...
        setIsDateRangeRegistrationOpen={setIsDateRangeRegistrationOpen}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-2">
                      {/* DEBUG: Always show join meeting button for testing */}
                      {/* {selectedSlot.slotData?.appointments?.[0] && (
                        <Button
                          size="sm"
                          className="text-xs bg-blue-600 hover:bg-blue-700 text-white"
                          disabled={isJoiningMeeting === selectedSlot.slotData.appointments[0].appointmentId}
                          onClick={() => selectedSlot.slotData?.appointments?.[0] && handleJoinMeeting(selectedSlot.slotData.appointments[0].appointmentId)}
                        >
                          {selectedSlot.slotData?.appointments?.[0] && isJoiningMeeting === selectedSlot.slotData.appointments[0].appointmentId ? (
                            <Loader2 className="w-3 h-3 mr-1 animate-spin" />
                          ) : (
                            <Video className="w-3 h-3 mr-1" />
                          )}
                          {selectedSlot.slotData?.appointments?.[0] && isJoiningMeeting === selectedSlot.slotData.appointments[0].appointmentId ? 'Đang tham gia...' : 'Tham gia cuộc hẹn (DEBUG)'}
                        </Button>
                      )} */}

                      {/* Join Meeting button for primary appointment if available */}
                      {(() => {
                        const primaryAppointment = selectedSlot.slotData?.appointments[0];
                        if (primaryAppointment) {
                          const { canJoin } = canJoinMeeting(primaryAppointment, selectedSlot.date, selectedSlot.timeSlot);
                          if (canJoin) {
                            return (
                              <Button
                                size="sm"
                                className="text-xs bg-green-600 hover:bg-green-700 text-white"
                                disabled={isJoiningMeeting === primaryAppointment.appointmentId}
                                onClick={() => handleJoinMeeting(primaryAppointment.appointmentId)}
                              >
                                {isJoiningMeeting === primaryAppointment.appointmentId ? (
                                  <Loader2 className="w-3 h-3 mr-1 animate-spin" />
                                ) : (
                                  <Video className="w-3 h-3 mr-1" />
                                )}
                                {isJoiningMeeting === primaryAppointment.appointmentId ? 'Đang tham gia...' : 'Tham gia cuộc hẹn'}
                              </Button>
                            );
                          }
                        }
                    ...
                    {selectedSlot.slotData.appointments.map((appointment, index) => (
                      ...
                      >
                        ...
                        <div className="flex space-x-2 mt-3">
                          ...
                          {(() => {
                            const { canJoin, reason } = canJoinMeeting(appointment, selectedSlot.date, selectedSlot.timeSlot);
                            return canJoin ? (
                              <Button
                                size="sm"
                                variant="default"
                                className="text-xs bg-blue-600 hover:bg-blue-700 text-white"
                                disabled={isJoiningMeeting === appointment.appointmentId}
                                onClick={() => handleJoinMeeting(appointment.appointmentId)}
                              >
                                {isJoiningMeeting === appointment.appointmentId ? (
                                  <Loader2 className="w-3 h-3 mr-1 animate-spin" />
                                ) : (
                                  <Video className="w-3 h-3 mr-1" />
                                )}
                                {isJoiningMeeting === appointment.appointmentId ? 'Đang tham gia...' : 'Tham gia cuộc hẹn'}
                              </Button>
      ...
      {/* Date Range Registration Modal */}
      <DateRangeRegistrationModal
        isOpen={isDateRangeRegistrationOpen}
        onClose={() => setIsDateRangeRegistrationOpen(false)}
