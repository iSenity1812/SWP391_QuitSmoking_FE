  ...
  useEffect(() => {
    const calculateTime = () => {
      ...
      if (isCountUp) {
        ...
        setCurrentDayOfPlan(
          QuitPlanCalculator.getDaysSinceStart(targetDate) + 1
        );
        // KẾT THÚC CHỈNH SỬA
      } else {
        ...
        // Tạo end of goal date (23:59:59.999 của ngày goalDate)
        const goalDate = new Date(targetDate);
        goalDate.setHours(23, 59, 59, 999); // Đặt về cuối ngày
        const goalTime = goalDate.getTime();

        const difference = goalTime - now.getTime();

        if (difference > 0) {
          // Tính lại số ngày dựa trên thời gian còn lại đến cuối ngày goalDate
          daysDiff = Math.floor(difference / (1000 * 60 * 60 * 24));
          hoursDiff = Math.floor(
        ...
        if (planStartDate) {
          setCurrentDayOfPlan(
            QuitPlanCalculator.getDaysSinceStart(planStartDate) + 1
          );
        } else {
    ...
    <div className="text-center">
      ...
      >
        ...
        <span className="font-bold text-lg">
          NGÀY {currentDayOfPlan.toString().padStart(2, "0")}
        </span>
      </motion.div>
