...
export class BlogService {
     ...
     */
    private static isSuccessResponse(response: ApiResponse<unknown>): boolean {
        ...
        try {
            ...
            if (typeof error === "object" && error !== null && "response" in error) {
                ...
                if (err.response.data?.message) {
                    throw new Error(`Backend error: ${err.response.data.message}`)
                }
            }
    ...
    static async getMyBlogs(page = 0, size = 10, sort = "createdAt,desc"): Promise<BlogPost[]> {
        try {
            ...
            if (typeof error === "object" && error !== null && "response" in error) {
                ...
                const err = error as { response: { status: number; data?: { message?: string } }; code?: string }
                console.error("Response error (MyBlogs):", err.response.status, err.response.data)
                ...
                if (
                    ...
                    if (err.response.data?.message) {
                        throw new Error(`Lỗi Backend: ${err.response.data.message}`)
                    }
                }
            }
    ...
    static async createBlog(blogData: CreateBlogRequest): Promise<BlogPost> {
        try {
            ...
            if (typeof error === "object" && error !== null) {
                const err = error as { name?: string; message?: string; response?: unknown; code?: string; request?: unknown }
                console.error("Error name:", err.name)
                console.error("Error message:", err.message)

                if (err.response) {
                    const resp = err.response as { status?: number; statusText?: string; data?: unknown; headers?: unknown }
                    console.error("Response status:", resp.status)
                    console.error("Response statusText:", resp.statusText)
                    ...
                    if (
                        ...
                        err.response !== null &&
                        "status" in err.response &&
                        (err.response as { status?: number }).status === 403
                    ) {
                    ...
                    if (
                        ...
                        err.response !== null &&
                        "status" in err.response &&
                        (err.response as { status?: number }).status === 400
                    ) {
                        // Bad request - could be validation error or wrong field name
                        const errorMessage = (err.response as { data?: { message?: string } }).data?.message || "Dữ liệu không hợp lệ"
                        console.error("400 Error details:", (err.response as { data?: unknown }).data)
                    ...
                    if (
                        ...
                        throw new Error(`Backend error: ${(err.response as { data?: { message?: string } }).data?.message}`)
                    }
                }
